...
In :mod:`loopy`, a :class:`loopy.program.Program` is a collection of callables
...
a :class:`~loopy.kernel.function_interface.InKernelCallable` is referred to as
resolving.


During code-generation process for a :class:`~loopy.program.Program`, a callable
...
:meth:`loopy.kernel.function_interface.InKernelCallable.with_dtypes` and
...
A user can *register* callables within a  :class:`~loopy.program.Program` to
...
a :class:`~loopy.target.Target`. Other foreign functions could be invoked by
*registering* them.
...
A :class:`loopy.kernel.LoopKernel` can be registered via
:func:`loopy.transform.callable.register_callable_kernel`.

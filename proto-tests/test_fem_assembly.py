...
def test_laplacian_stiffness(ctx_factory):
    ...
    Nq = 40  # num. quadrature points (baked into code)  # ruff:ignore[non-lowercase-variable-in-function]
    Nb = 20  # num. basis functions (baked into code)  # ruff:ignore[non-lowercase-variable-in-function]
    Nc = 100  # num. cells (run-time symbolic)  # ruff:ignore[non-lowercase-variable-in-function]

    from pymbolic import var
    Nc_sym = var("Nc")  # ruff:ignore[non-lowercase-variable-in-function]

    knl = lp.make_kernel(ctx.devices[0],
            "[Nc] -> {[K,i,j,q, dx_axis, ax_b]: 0<=K<Nc and 0<=i,j<%(Nb)d and 0<=q<%(Nq)d "  # ruff:ignore[line-too-long]
    ...
    def variant_pg4(knl):
        ...
        knl = lp.tag_inames(knl, {"dx_axis": "unr"})
        Ncloc = 16  # ruff:ignore[non-lowercase-variable-in-function]

...
def test_laplacian(ctx_factory):
    ...
    knl = lp.make_kernel(ctx.devices[0],
            "[K] -> {[i,j,k,e,m,o1,o2,o3,gi]: 0<=i,j,k,m,o1,o2,o3<%d and 0<=e<K and 0<=gi<6}" % n,  # ruff:ignore[line-too-long]
            [
                ...
                # define function
                "CSE: Gu(i,j,k) = G[0,e,i,j,k]*ur(i,j,k) + G[1,e,i,j,k]*us(i,j,k) + G[2,e,i,j,k]*ut(i,j,k)",  # ruff:ignore[line-too-long]
    ...
    if 0:
        pass
        # seq_knl = lp.add_prefetch(knl, "G", ["gi", "m", "j", "k"], "G[gi,e,m,j,k]", default_tag="l.auto")  # ruff:ignore[line-too-long]
        # seq_knl = lp.add_prefetch(seq_knl, "D", ["m", "j"], default_tag="l.auto")
        # seq_knl = lp.add_prefetch(seq_knl, "u", ["i", "j", "k"], "u[*,i,j,k]", default_tag="l.auto")  # ruff:ignore[line-too-long]
    else:
        seq_knl = knl
    ...
    knl = lp.add_prefetch(knl, "D", ["m", "j"],
            default_tag="l.auto")
    # knl = lp.add_prefetch(knl, "u", ["i", "j", "k"], "u[*,i,j,k]", default_tag="l.auto")  # ruff:ignore[line-too-long]
...
def test_laplacian_lmem(ctx_factory):
    ...
    # knl = lp.add_prefetch(knl, "G", ["i", "j", "m", "k"], default_tag="l.auto") # axis/argument indices on G  # ruff:ignore[line-too-long]
    # print(knl)
    # 1/0
...
def test_laplacian_lmem_ilp(ctx_factory):
    ...
    knl = lp.precompute(knl, "ur", np.float32, [0, 1, 2, "e_inner_inner"], default_tag="l.auto")  # ruff:ignore[line-too-long]
    knl = lp.precompute(knl, "us", np.float32, [0, 1, 2, "e_inner_inner"], default_tag="l.auto")  # ruff:ignore[line-too-long]
    knl = lp.precompute(knl, "ut", np.float32, [0, 1, 2, "e_inner_inner"], default_tag="l.auto")  # ruff:ignore[line-too-long]

    knl = lp.add_prefetch(knl, "G", ["m", "i", "j", "k", "e_inner_inner"], default_tag="l.auto")  # ruff:ignore[line-too-long]
...
def test_advect(ctx_factory):
    ...
    N = 8  # ruff:ignore[non-lowercase-variable-in-function]
    ...
    knl = lp.make_kernel(ctx.devices[0],
            ...
            [
                ...
                # CSE?
                "CSE: Vr(i,j,k) = G[0,e,i,j,k]*u[e,i,j,k] + G[1,e,i,j,k]*v[e,i,j,k] + G[2,e,i,j,k]*w[e,i,j,k]",  # ruff:ignore[line-too-long]
                "CSE: Vs(i,j,k) = G[3,e,i,j,k]*u[e,i,j,k] + G[4,e,i,j,k]*v[e,i,j,k] + G[5,e,i,j,k]*w[e,i,j,k]",  # ruff:ignore[line-too-long]
                "CSE: Vt(i,j,k) = G[6,e,i,j,k]*u[e,i,j,k] + G[7,e,i,j,k]*v[e,i,j,k] + G[8,e,i,j,k]*w[e,i,j,k]",  # ruff:ignore[line-too-long]

                # form nonlinear term on integration nodes
                "Nu[e,i,j,k] = Vr(i,j,k)*ur(i,j,k)+Vs(i,j,k)*us(i,j,k)+Vt(i,j,k)*ut(i,j,k)",  # ruff:ignore[line-too-long]
                "Nv[e,i,j,k] = Vr(i,j,k)*vr(i,j,k)+Vs(i,j,k)*vs(i,j,k)+Vt(i,j,k)*vt(i,j,k)",  # ruff:ignore[line-too-long]
                "Nw[e,i,j,k] = Vr(i,j,k)*wr(i,j,k)+Vs(i,j,k)*ws(i,j,k)+Vt(i,j,k)*wt(i,j,k)",  # ruff:ignore[line-too-long]
                ],
            [
            lp.ArrayArg("u", dtype, shape=field_shape, order=order),
...
def test_advect_dealias(ctx_factory):
    ...
    N = 8  # ruff:ignore[non-lowercase-variable-in-function]
    M = 8  # ruff:ignore[non-lowercase-variable-in-function]
    ...
    # K - run-time symbolic
    knl = lp.make_kernel(ctx.devices[0],
            "[K] -> {[i,ip,j,jp,k,kp,m,e]: 0<=i,j,k,m<%d AND 0<=o,ip,jp,kp<%d 0<=e<K}" % (M, N),  # ruff:ignore[line-too-long]
            [
                ...
                "CSE: Vr[i,j,k,e] = G[i,j,k,0,e]*Iu[i,j,k,e] + G[i,j,k,1,e]*Iv[i,j,k,e] + G[i,j,k,2,e]*Iw[i,j,k,e]",  # ruff:ignore[line-too-long]
                "CSE: Vs[i,j,k,e] = G[i,j,k,3,e]*Iu[i,j,k,e] + G[i,j,k,4,e]*Iv[i,j,k,e] + G[i,j,k,5,e]*Iw[i,j,k,e]",  # ruff:ignore[line-too-long]
                "CSE: Vt[i,j,k,e] = G[i,j,k,6,e]*Iu[i,j,k,e] + G[i,j,k,7,e]*Iv[i,j,k,e] + G[i,j,k,8,e]*Iw[i,j,k,e]",  # ruff:ignore[line-too-long]

                # form nonlinear term on integration nodes
                # QUESTION: should I use CSE here ?
                "<SE: Nu[i,j,k,e] = Vr[i,j,k,e]*Iur[i,j,k,e]+Vs[i,j,k,e]*Ius[i,j,k,e]+Vt[i,j,k,e]*Iut[i,j,k,e]",  # ruff:ignore[line-too-long]
                "<SE: Nv[i,j,k,e] = Vr[i,j,k,e]*Ivr[i,j,k,e]+Vs[i,j,k,e]*Ivs[i,j,k,e]+Vt[i,j,k,e]*Ivt[i,j,k,e]",  # ruff:ignore[line-too-long]
                "<SE: Nw[i,j,k,e] = Vr[i,j,k,e]*Iwr[i,j,k,e]+Vs[i,j,k,e]*Iws[i,j,k,e]+Vt[i,j,k,e]*Iwt[i,j,k,e]",  # ruff:ignore[line-too-long]
...
def test_interp_diff(ctx_factory):
    ...
    # K - run-time symbolic
    knl = lp.make_kernel(ctx.devices[0],
            "[K] -> {[i,ip,j,jp,k,kp,e]: 0<=i,j,k<%d AND 0<=ip,jp,kp<%d 0<=e<K}" % (M, N),  # ruff:ignore[line-too-long]
            [
                "[|i,jp,kp] <float32>  u1[i ,jp,kp,e] = sum_float32(ip, I[i,ip]*u [ip,jp,kp,e])",  # ruff:ignore[line-too-long]
                "[|i,j ,kp] <float32>  u2[i ,j ,kp,e] = sum_float32(jp, I[j,jp]*u1[i ,jp,kp,e])",  # ruff:ignore[line-too-long]
                "[|i,j ,k ] <float32>  u3[i ,j ,k ,e] = sum_float32(kp, I[k,kp]*u2[i ,j ,kp,e])",  # ruff:ignore[line-too-long]
                "[|i,j ,k ] <float32>  Pu[i ,j ,k ,e] = P[i,j,k,e]*u3[i,j,k,e]",
                "[|i,j ,kp] <float32> Pu3[i ,j ,kp,e] = sum_float32(k, V[kp,k]*Pu[i ,j , k,e])",  # ruff:ignore[line-too-long]
                "[|i,jp,kp] <float32> Pu2[i ,jp,kp,e] = sum_float32(j, V[jp,j]*Pu[i ,j ,kp,e])",  # ruff:ignore[line-too-long]
